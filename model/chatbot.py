...
class ChatBot:
  CHATBOT_NAME= "BudgetBud Advisor"
  ...
  def send_prompt(self, prompt, temperature = 0.1):
    if temperature< 0  or temperature > 1:
      raise GenAIException("Temperature must be between o and 1")
    if not prompt:
  ...
  def preload_conversation(self, conversation_history = None):
    if isinstance(conversation_history, list):
      ...
      self._conversation_history = [
        self._construct_message('From now on, return the ouput as a JSON object that can be loaded in Python with the key as \'text\'. For example, {"text": "<output goes here>"}'),
        self._construct_message('{"text": "Sure, I can return the ouput as a regular JSON with the key as `text`. Here is an exaple: {"text": "Your Output"}.', 'model')
      ]
